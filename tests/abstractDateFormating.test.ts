...
describe.concurrent("Abstract Date Formatting", () => {
  ...
  test("[formatAbstractDate] - abstract date", () => {
    ...
    expect(formatAbstractDate(absctractDateMock, settings)).toBe("00/00/-1000");
    settings.dateDisplayFormat = "{month}-{year}-{day}";
    expect(formatAbstractDate(absctractDateMock, settings)).toBe("00--1000-00");
  ...
  test("[applyConditionBasedFormatting] - ok full conditions exclusive", () => {
    ...
    expect(applyConditionBasedFormatting(input, -56, configuration, true)).toBe(
      `${input} sample condition formatting`,
    );

    expect(applyConditionBasedFormatting(input, 0, configuration, true)).toBe(
      input,
    );
  });
