...
describe.concurrent("Suggester", () => {
  ...
  test("[TimelineMarkdownSuggester] - ok getSuggestions TagToFind", () => {
    ...
    app.metadataCache.getFileCache = mockGetFileCache().mockImplementationOnce(
      () => ({
  ...
  test("[TimelineMarkdownSuggester] - ko selectSuggestion", () => {
    ...
    expect(suggester.selectSuggestion("", mock<MouseEvent>())).toBeUndefined();
  });
   ...
   * @param type - the passed type of the suggestion
