...
/**
 * Provides additional context for the creation cards in the DOM but reads it from the body using @javalent the-calendarium's events (https://plugins.javalent.com/calendarium/events/automatic#Span+tag+event+creation).
 *
 * @param body - The extracted body for a single event card.
 * @param context - Timeline generic context.
 * @param tagsToFind - The tags to find in a note to match the current timeline.
...
export async function getDataFromFantasyCalendarSpanEvents(
  ...
  for (const element of nodeList) {
    const fakeFrontmatter = attributesToLookFor.reduce(
      (accumulator, key) => {
        accumulator[attributeDictionary[key]] = element.getAttribute(key);
        return accumulator;
      },
