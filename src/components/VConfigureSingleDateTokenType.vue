  ...
  <div class="v-grid-display">
    ...
    >
      <template #description>
        {{
          $t(`settings.description.configureSingleDateToken.${modelValue.type}`)
        }}
      </template>
    </VSelect>
    <Transition mode="out-in">
      ...
      >
        ...
        >
          <template #label>{{
            $t("settings.label.configureSingleDateTokenType.minLength")
          }}</template>
          <template #description>{{
            $t("settings.description.configureSingleDateTokenType.minLength")
          }}</template>
        </VInput>
        <VCheckbox
        ...
        >
          <template #label>{{
            $t("settings.label.configureSingleDateTokenType.hideSign")
          }}</template>
          <template #description>{{
            $t("settings.description.configureSingleDateTokenType.hideSign")
          }}</template>
        </VCheckbox>
      </div>
      <ul
      ...
      >
        ...
        >
          <VInput
            ...
            :model-value="entry"
            :placeholder="$t('settings.placeholder.stringTokenDctionaryEntry')"
            @update:model-value="handleDictionaryUpdateAtIndex(index, $event)"
          />
          <VButton
