  ...
  <div class="v-grid-display">
    ...
    <section class="v-grid-display slim" v-for="key in checkboxKeys">
      ...
      >
        <template #label>{{ $t(`settings.label.${key}`) }}</template>
        <template #description>
          <i18n-t scope="global" :keypath="`settings.description.${key}`">
            <template
              #linkText
              v-if="
            ...
            <template #link>
              <b
                ><a
                  ...
                  target="_blank"
                >
                  {{ $t("settings.label.fantasyCalendarSpanEvents.link") }}
                </a></b
              >
            </template>
          </i18n-t>
    ...
    <section class="v-grid-display">
      ...
      <VToggleNumber
        ...
        :input-id="key"
      >
        <template #checkbox-label>{{ $t(`settings.label.${key}`) }}</template>
      </VToggleNumber>
    </section>
  </div>
</template>
